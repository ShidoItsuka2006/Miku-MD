...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    Konichiwa ${pushname} senpai 👋. I am ${global.BotName}, a bot developed by: Shido to take your WhatsApp usage into next level.
...
case 'handsomecheck':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
				if (!text) return replay(`Tag Someone, Example : ${prefix + command} @Shido`)
...
I am *Miku Nakano*, a bot developed by *Shido*.
